...
; RB5 == led output
#define	OUTPUT_LED 		PORTB, RB5	; Define the OUTPUT LED PIN  0/off, 1/on
...
Delay_Count 	res 1		; Define registers for
Delay_Count2 	res 1		; delay routines (0x10,0x11 for 10F20x)

DataByte		res 1		; Define a byte to use for RC5 Data
AddrByte		res 1		; Define a byte to use for RC5 address
ToggByte		res 1		; Define a byte to determine toggle low or high
...
SendRC5
    bcf INTCON, GIEH        ; disable interupts for the duration

	; Pre-shift Addr Byte
    rlcf AddrByte, F		; Must be rotated left 3 bits
    rlcf AddrByte, F		;
	rlcf AddrByte, F		; MSB now is MSB of 5 bit #
	; Pre-shift Data Byte
	rlcf DataByte, F		; Must be rotated left 2 bits
	rlcf DataByte, F		; MSB now is MSB of 6 bit #
	...
	; bit 4
	rlcf		AddrByte, F		; Shift out MSB.. C = MSB
	...
	; Shift Out DataByte
	; bit 5
	rlcf		DataByte, F		; Shift out MSB.. C = MSB
	btfss	STATUS, C		; if bit is 1, skip next instr.
	call 	SendZero		; bit is 0, send a zero
	btfsc	STATUS, C
	call	SendOne			; bit is 1, send a one
...
;
    bsf INTCON, GIEH            ; enable interupts
